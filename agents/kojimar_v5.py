...
# Forward-search imports are optional: if the SDK build differs, search stays
# off and the agent runs as the pure (1033-rated) heuristic.
try:
...
USE_SEARCH = True
SEARCH_CANDIDATES = 4     # top heuristic moves to look ahead on
SEARCH_BUDGET = 0.06      # seconds per decision (timeout-safe)
SEARCH_TURN_STEPS = 16    # cap on simulated steps to finish our own turn
...
# Pokemon whose ability prevents ALL damage from the opponent's ex Pokemon.
# Mega Lucario ex deals 0 to these, so we must attack them with a non-ex
# Pokemon (Hariyama / Solrock) instead. This is the fix for the 0.26 crustle
# matchup: the deck already has the tools; the agent just has to model the
# ability and pick the non-ex attacker into the wall.
EX_DAMAGE_BLOCKERS = {345}  # Crustle (Mysterious Rock Inn)
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    # If the target prevents all damage from ex Pokemon and our
                    # attacker is an ex, this attack does nothing. Zero it so the
                    # planner prefers our non-ex attackers (Hariyama/Solrock).
                    my_data = card_table[my_pokemon.id]
                    if op_pokemon.id in EX_DAMAGE_BLOCKERS and (my_data.ex or my_data.megaEx):
                        damage = 0
...
def _eval_state(obs: Observation, my_index: int) -> float:
    """Value of a (simulated) position from our perspective. Prizes decide the
    game; reward taking the opponent's prizes and keeping our own, plus board
    presence and active-attacker health."""
    state = obs.current
    if state is None:
        return 0.0
    ...
    val = (len(opp.prize) - len(me.prize)) * 100000.0   # prize race dominates
    # opponent board damage taken is good; our board health is good
    ...
    for p in me.active + me.bench:
        if p is not None:
            val += p.hp * 2
    val += len(me.bench) * 300
    return val


def _complete_turn_value(sid, first_obs, my_index) -> float:
    """From a stepped search state, finish our own turn greedily with the same
    heuristic, then return the resulting position value."""
    cur = first_obs
    steps = 0
    while steps < SEARCH_TURN_STEPS:
        st = cur.current
        if st is None or (st.result is not None and st.result >= 0):
            break
        if cur.select is None:
            break
        if st.yourIndex != my_index:
            break  # opponent's turn — stop, evaluate here
        try:
            sub = LucarioPolicy(cur).choose()
        except Exception:
            break
        if not sub:
            sub = list(range(cur.select.minCount)) or [0]
        ar = search_step(sid, sub)
        nxt = ar.observation if hasattr(ar, "observation") else getattr(getattr(ar, "state", None), "observation", None)
        if nxt is None:
            break
        cur = nxt
        steps += 1
...
def _search_pick(obs: Observation, base_order: list[int]) -> list[int] | None:
    """Look ahead on the top heuristic candidates; keep the move whose finished
    turn evaluates best. Returns None to signal 'use the heuristic'."""
    if not (_SEARCH_OK and USE_SEARCH):
        return None
    select = obs.select
    state = obs.current
    if (select.context != SelectContext.MAIN or select.maxCount != 1
            or select.minCount != 1 or len(select.option) < 2):
        return None
    ...
    me = state.players[my_index]
    opp = state.players[1 - my_index]
    cands = [i for i in base_order[:SEARCH_CANDIDATES]]
    ...
    for first in cands:
        ...
        try:
            ss = search_begin(
                obs,
                your_deck=(random.sample(my_deck, me.deckCount)
                           if me.deckCount <= len(my_deck) else my_deck[:me.deckCount]),
                your_prize=(random.sample(my_deck, len(me.prize))
                            if len(me.prize) <= len(my_deck) else my_deck[:len(me.prize)]),
                opponent_deck=[1072] * opp.deckCount,
                opponent_prize=[1] * len(opp.prize),
                opponent_hand=[1] * opp.handCount,
                opponent_active=([opp.active[0].id]
                                 if (opp.active and opp.active[0] is not None) else []),
            )
            sid = ss.searchId
            ar = search_step(sid, [first])
            nxt = ar.observation if hasattr(ar, "observation") else getattr(getattr(ar, "state", None), "observation", None)
            if nxt is None:
                continue
            v = _complete_turn_value(sid, nxt, my_index)
            if v > best_v:
                best_v, best_i = v, first
        except Exception:
            return None  # any failure -> heuristic
        finally:
            try:
                if sid is not None:
                    search_end()
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        order = LucarioPolicy(obs).choose()
    except Exception:
        # crash-safe legal fallback
        sel = obs.select
        return list(range(sel.minCount)) if sel.minCount > 0 else [0]

    # Try forward search on the heuristic's top candidates; fall back to it.
    try:
        searched = _search_pick(obs, order)
