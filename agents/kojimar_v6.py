...
# Finish-mode forward search: only fires when a win MAY be available this turn.
USE_SEARCH = True
SEARCH_BUDGET = 0.10   # seconds; under the per-decision limit
SEARCH_MAX_STEPS = 20
...
class PrizeTracker:
    """Conservative prize deduction. Returns None when ambiguous — a wrong
    guess makes search hallucinate winning lines the real game can't repeat."""

    def __init__(self, decklist):
        self._deck = Counter(int(c) for c in decklist)
        self._prized = None
    ...
    def update(self, obs, obs_dict):
        ...
        if (self._prized is not None and self._last_prize_count is not None
            ...
            taken = [lg["cardId"] for lg in obs_dict.get("logs", [])
                     if lg.get("fromArea") in _PRIZE_AREA and "cardId" in lg]
            if not taken:
                self._prized = None
            else:
                rem = Counter(taken)
                if any(self._prized.get(c, 0) < n for c, n in rem.items()):
                    self._prized = None
                else:
        ...
        inferred += Counter()
        if sum(inferred.values()) == pc and pc > 0:
            self._prized = inferred

    def hidden_deck(self, deck_count):
        if self._prized is None:
            return None
        ids = list((self._deck - self._prized).elements())
...
def _verified_win_move(obs: Observation) -> list[int] | None:
    """Finish mode: only when a win may be reachable this turn, simulate our
    own turn forward (completed by the same heuristic) and, if it reaches a
    win, return the first move of that verified line. Feeds search the REAL
    deck minus inferred prizes so it can't invent lines using prized cards.
    Returns None to fall back to the rule policy."""
    if not (_SEARCH_OK and USE_SEARCH):
        return None
    sel = obs.select
    state = obs.current
    if sel is None or sel.context != SelectContext.MAIN or sel.minCount != 1 or sel.maxCount != 1:
        return None
    my = state.yourIndex
    me = state.players[my]
    opp = state.players[1 - my]
    # Only bother when winning is plausibly close: opponent near 0 prizes.
    if len(opp.prize) > 2:
        return None
    if obs.search_begin_input is None:
        return None
    hidden = _tracker.hidden_deck(me.deckCount) if _tracker is not None else None
    if hidden is None:
        return None  # prizes unknown -> do NOT search (avoid NOMATCH)

    order = LucarioPolicy(obs).choose()
    t0 = time.time()
    for first in order[: min(5, len(order))]:
        ...
        try:
            ss = search_begin(
                obs,
                your_deck=hidden[: me.deckCount],
                your_prize=hidden[: len(me.prize)] or hidden[:1] * len(me.prize),
            ...
            sid = ss.searchId
            cur = search_step(sid, [first]).observation
            steps = 0
            won = False
            while steps < SEARCH_MAX_STEPS:
                st = cur.current
                if st is None:
                    break
                if st.result is not None and st.result >= 0:
                    won = (st.result == my)
                    break
                if cur.select is None or st.yourIndex != my:
                    break  # opponent's turn reached without a win
                nxt = LucarioPolicy(cur).choose()
                if not nxt:
                    nxt = list(range(cur.select.minCount)) or [0]
                cur = search_step(sid, nxt).observation
                steps += 1
            if won:
                return [first]
        except Exception:
