...
def agent(obs_dict: dict) -> list[int]:
    ...
    for o in select.option:
        ...
        if context == SelectContext.MAIN:
            if o.type == OptionType.ATTACK:
                # Attack is central, not a last resort. Taking prizes wins the
                # game; a lethal KO is the top priority.
                score = 3000
                if lethal:
                    score = 90000
                if opp_prize <= 1:
                    score += 5000  # opponent close to losing -> press
    ...
    for i in order:
        if len(out) >= select.maxCount:
            break
        if scores[i] >= 0 or len(out) < select.minCount:
            out.append(i)
    # guarantee legality
